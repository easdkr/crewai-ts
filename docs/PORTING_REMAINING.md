...
- Flow event causal ordering is release-gated for listener chains, router paths, and parallel listeners: downstream method execution events preserve upstream `triggered_by_event_id` links to the exact method completion event that caused them.
