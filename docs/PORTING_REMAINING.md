...
- Crew agent parser JSON repair is release-gated for upstream ReAct parsing edge cases: incomplete JSON object tool inputs close open strings and braces before normalization, and unbalanced quoted non-JSON tool inputs are preserved without the stray leading quote.
