...
- Hierarchical manager delegation tools are release-gated for upstream coworker scoping and tool descriptions: assigned-task delegation exposes only that task's agent, unassigned tasks expose all crew agents, and manager tool descriptions use the upstream "specific task/question" coworker wording.
