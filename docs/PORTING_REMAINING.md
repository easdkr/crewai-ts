...
- `ReadFileTool` typed-file behavior is release-gated for upstream metadata, required `file_name` args schema, no-file and missing-file messages, multiple text/JSON file lookup, ignored extra kwargs, binary base64 output, and PDF text extraction through the local `pdf-parse` dependency with explicit binary/error fallbacks when extraction is unavailable or fails.
