...
- Added adapter-level LLM provider parity helpers:
  ...
  - Anthropic completion shim now exposes upstream-style SDK response token usage extraction, tool-use/structured-output response extraction, and deterministic streaming event accumulation, including cache read and cache creation token fields
  ...
  - Bedrock completion shim now exposes upstream-style Converse token usage extraction/tracking, tool-use/structured-output response extraction, and deterministic Converse streaming event accumulation, including cache read token fields
  - Gemini completion shim with upstream-style message formatting, generation config builders, tool conversion, function-call and structured-output response extraction, deterministic streaming chunk accumulation, config, context-window, multimodal/text-formatting, token-usage extraction, response text extraction, property ordering, and content conversion helpers
...
2. LLM providers
   ...
   - `to_config_dict`, context window, adapter-level function-calling support, deterministic multimodal support flags, response-chain/reset compatibility, file input content-block conversion, local uploader compatibility, streaming tool-call accumulation, OpenAI/Anthropic/Bedrock SDK usage extraction, OpenAI Responses built-in output parsing and streaming event accumulation, Anthropic/Bedrock event accumulation, Gemini streaming chunk accumulation, Anthropic/Bedrock/Gemini function-call/structured-output response extraction, and OpenAI/Azure/Anthropic/Bedrock/Gemini request builders are now covered for the native provider shims.
