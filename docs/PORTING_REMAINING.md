...
- RAG embedding provider backward compatibility is release-gated for the upstream `google` provider alias: `buildEmbedderFromDict` resolves it to the Google Generative AI provider, accepts the legacy `model` key, preserves task type, and builds deterministic request payloads with a mocked fetch.
