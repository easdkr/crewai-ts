...
- Added adapter-level LLM provider parity helpers:
  ...
  - OpenAI native completion shim now exposes upstream-style SDK response token usage extraction, Responses API result value-object helpers, parse-tool-output and auto-chain flags, explicit previous-response precedence, output parsing for function calls, built-in tool outputs, and reasoning items, and deterministic Responses streaming event accumulation, including cached prompt tokens and reasoning tokens
