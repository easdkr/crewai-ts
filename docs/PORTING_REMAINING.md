...
- The same external examples audit promoted the deterministic `crewAI-examples` write-a-book nested crew shape: `OutlineCrew` produces structured `BookOutline` chapters with Serper-style research attached, `WriteBookChapterCrew` consumes serialized full-outline context for each chapter and returns structured markdown `Chapter` output without Serper, provider calls, file writes, markdown artifact generation, or pydantic runtime dependencies.
...
- The same external examples audit promoted the deterministic `crewAI-examples` email auto-responder CrewBase/tool shape: `EmailFilterCrew` auto-collects three sequential agents/tasks, preserves Serper, Gmail-thread, Tavily, and CreateDraft tool boundaries plus the filter/action/writer context handoff, and the CreateDraft helper converts pipe-separated email/subject/message input into a Gmail draft payload without LangChain, GmailToolkit, live Gmail API resources, provider calls, or real draft creation.
...
- The same external examples audit promoted the deterministic `crewAI-examples` industry-specialized agents notebook shape: biomedical, healthcare, and finance agents all attach Serper/Weaviate-style tools, share the same Weaviate feature research task shape, and run `kickoff_for_each` across MUVERA, Multi-tenancy, Compliance, and Hybrid Search inputs without Weaviate Cloud credentials, vector search, Serper, OpenAI, dotenv, notebook package installs, or network calls.
- The same external examples audit promoted the deterministic `crewAI-examples` game-builder CrewBase template shape: three sequential code/review/evaluate tasks gate automatic agent/task collection, `allow_delegation` false/false/true agent settings, kickoff `{game}` interpolation, and training iteration/filename/input propagation without YAML reads, provider calls, or generated game artifacts.
- The same external examples audit promoted the deterministic `crewAI-examples` markdown-validator CrewBase workflow shape: a no-delegation requirements manager with a `markdown_validation_tool`, single sequential syntax review task, filename-required run wrapper, kickoff `{query, filename}` interpolation, and formatted clean/error scan results are gated without `pymarkdown`, LangChain, OpenAI, or remote provider calls.
- The same external examples audit promoted the deterministic `crewAI-examples` marketing-strategy CrewBase workflow shape: market analyst, strategist, and creator agents gate fake Serper/scrape tool attachment, five sequential tasks, structured `output_json` schemas for strategy/campaign/copy, copy task context dependencies, and `{customer_domain, project_description}` interpolation without live search, scraping, memory, or provider calls.
- The same external examples audit promoted the deterministic `crewAI-examples` landing-page generator workflow shape: Expand/Choose/Create CrewBase projects run sequentially, fake search/scrape/template/file-management tools attach to the React engineer, choose-output strings are sanitized into component lists, and CreateContent processes only existing safe component filenames without templates, Browserless, Serper, LangChain file tooling, zipping, or generated site artifacts.
- The same external examples audit promoted the deterministic `crewAI-examples` match-profile-to-positions CrewBase workflow shape: CV reader and matcher agents gate FileRead/CSVSearch-style tool attachment, two sequential tasks, `{path_to_cv, path_to_jobs_csv}` interpolation, and local CV/job CSV matching boundaries without `crewai_tools`, vector search, external databases, or provider calls.
- The same external examples audit promoted the deterministic `crewAI-examples` starter-template manual crew workflow shape: separate agent/task factory classes, explicit `Crew(agents=[...], tasks=[...])` construction, default sequential process behavior, var1/var2 task interpolation, and shared tip text are gated without decouple, LangChain/OpenAI/Ollama clients, DuckDuckGo search, or interactive stdin.
- The same external examples audit promoted the deterministic `crewAI-examples` instagram-post two-crew workflow shape: manual agent/task factories build a copy crew and a follow-on image crew, fake scrape/search/Instagram tools attach to the right agents, product website/details interpolate through analysis/campaign/photo tasks, and the first crew's ad-copy output feeds the photographer task without dotenv, Ollama, Browserless, Serper, unstructured HTML parsing, or interactive stdin.
- The same external examples audit promoted the deterministic `crewAI-examples` prep-for-a-meeting manual crew workflow shape: four manual meeting-prep agents share Exa-style search/find-similar/get-contents tools, async research and industry tasks feed strategy and briefing context, participant/context/objective inputs interpolate through prompts, and explicit `Crew(agents=[...], tasks=[...])` construction is gated without dotenv, Exa, LangChain, provider calls, network search, or interactive stdin.
- The same external examples audit promoted the deterministic `crewAI-examples` meta-quest knowledge CrewBase workflow shape: a PDFKnowledgeSource is attached as crew knowledge, the Meta Quest expert task interpolates `{question}`, and retrieved manual context is injected into the prompt without live PDF parsing dependencies, provider calls, or external knowledge storage.
- The same external examples audit promoted the deterministic `crewAI-examples` recruitment CrewBase workflow shape: researcher, matcher, communicator, and reporter agents gate Serper/Scrape/LinkedIn-style tool attachment, job requirements interpolate through the first three tasks, and the final recruiter report task consumes research, scoring, and outreach context without Selenium, LinkedIn, live search, scraping, provider calls, or external profile services.
- The same external examples audit promoted the deterministic `crewAI-examples` stock-analysis CrewBase workflow shape: effective CrewBase task graph includes research, financial, and investment advisor agents plus research/financial/filings/recommend tasks, fake website/search/calculator/SEC 10-Q/10-K tools attach to the expected agents, `{company_stock}` interpolates through stock prompts, and the recommendation task receives prior research, financial, and filings outputs without dotenv, Ollama, SEC API, HTTP fetches, RagTool storage, provider calls, or live market data.
- The same external examples audit promoted the deterministic `crewAI-examples` surprise-trip CrewBase workflow shape: activity planner, restaurant scout, and itinerary compiler agents gate Serper/Scrape-style tool attachment, traveler inputs interpolate through activity and restaurant prompts, and the final itinerary task uses structured `output_json`/response-model output while consuming prior activity and dining context without live search, scraping, provider calls, or generated travel artifacts.
- The same external examples audit promoted the deterministic `crewAI-examples` trip-planner manual crew workflow shape: separate trip agent/task factories construct city-selection, local-expert, and travel-concierge agents with search/browser/calculator-style tools, origin/city/date/interests inputs interpolate through identify/gather/plan tasks, shared tip text is preserved, and the final itinerary task consumes earlier city-selection and local-guide outputs without dotenv, OpenAI/LangChain clients, Serper, Browserless, unstructured HTML parsing, network calls, or interactive stdin.
- The same external examples audit promoted the deterministic `crewAI-examples` screenplay-writer script workflow shape: a standalone spam-filter task gates processing, analyst/scriptwriter/formatter agents run as a sequential crew, parenthetical directions are stripped from the formatted script, and a scorer task parses the first score line without dotenv, YAML file loading, provider calls, or interactive stdin.
- The same external examples audit promoted the deterministic `crewAI-examples` Azure model integration shape: AzureChatOpenAI-style env config is injected into a Senior Researcher agent, a single research task runs in a sequential crew, and kickoff prompt handoff is gated without dotenv, LangChain Azure clients, Azure credentials, network calls, or live provider execution.
- The same external examples audit promoted the deterministic `crewAI-examples` NVIDIA NIM intro integration shape: env-derived model/base URL/API key config flows through a ChatNVIDIA-style shim and custom `nvllm` wrapper into a Senior Researcher sequential crew, preserving invoke payload shape without dotenv, LiteLLM callback mutation, LangChain NVIDIA clients, NVIDIA credentials, network calls, or live provider execution.
- The same external examples audit promoted the deterministic `crewAI-examples` NVIDIA marketing-strategy integration shape: NVIDIA `nvllm` wrapper config feeds a CrewBase marketing workflow, Serper/Scrape-style tools attach to analyst/strategist agents, structured `output_pydantic` strategy/campaign/copy tasks export parsed outputs, and final copy consumes strategy/campaign context without dotenv, LangChain NVIDIA clients, NVIDIA credentials, network calls, live provider execution, or LiteLLM callback mutation.
- The same external examples audit promoted the deterministic `crewAI-examples` CrewAI-LangGraph email workflow shape: a StateGraph-style workflow wires check_new_emails, conditional continue/end routing, draft_responses, and wait_next_run nodes; Gmail-like filtering skips checked, duplicate-thread, and self-sent emails; and an EmailFilterCrew drafts responses from formatted email context without dotenv, LangGraph, GmailToolkit, Gmail/Tavily tools, network calls, real sleeps, or live provider execution.
- The same external examples audit promoted the deterministic `crewAI-examples` Flows 101 content-router notebook workflow shape: a class-level `@persist(verbose=True)` Flow collects URL/content type state, a router emits `blog`/`newsletter`/`linkedin`, only the selected listener builds a two-task research/writing crew with context handoff and final content state, and persistence calls are captured by a fake local store without interactive input, Serper, provider calls, notebook execution, network access, or live SQLite persistence.
- The same external examples audit promoted the deterministic `crewAI-examples` Coding Assistant notebook Flow shape: initial code generation feeds router-based import/code execution checks, failed imports route to `fix_code`, failed code blocks route to `refix_code`, fix prompts include previous error/question/explanation/imports/code/context, and the final listener rechecks a working solution without LangGraph, WebsiteSearchTool, provider calls, unsafe notebook execution, CSV evaluation files, or live LCEL documentation retrieval.
- The same external examples audit promoted the deterministic `crewAI-examples` QA Agent / Simple QA Crew+Flow notebook shape: a `QAState` start method rewrites the user question through an AI Suite-style client boundary, a method-reference listener passes the improved question into a WebsiteSearch-backed crew, and the resulting `CrewOutput.raw` is stored as `state.answer` without notebook package installs, API-key prompts, live WebsiteSearchTool retrieval, provider calls, graph plotting, or generated HTML display.
- The same external examples audit promoted the deterministic `crewAI-examples` QA Agent LangGraph RAG notebook workflow shape: an agent node selects a retriever tool, retrieved documents are graded into `generate` or `rewrite` routes, a rewritten question loops back through the agent/retriever path, and relevant context finally feeds generation without LangChain/LangGraph packages, Chroma/OpenAI embeddings, web loading, LangChain Hub prompts, provider streaming, graph rendering, or live notebook execution.
- The same external examples audit promoted the deterministic `crewAI-examples` Coding Assistant LangGraph/evaluation notebook shape: generate/check/decide loops append retry prompts after import and code-execution failures, stop on success before the max-iteration cap, and CrewAI-vs-LangGraph import/execution pass-rate metrics are rounded to two decimals without LangGraph packages, pandas, unsafe `exec`, provider calls, CSV files, or live LCEL documentation retrieval.
...
- The latest-upstream `Agent.kickoff(response_format=...)` audit is now covered deterministically: direct Agent kickoff accepts `response_format` / `responseFormat` aliases, forwards the schema as the provider response model, and returns `LiteAgentOutput` with parsed `pydantic` content only when structured output is requested, preserving legacy raw-string kickoff calls otherwise.
...
- The latest-upstream streaming exception audit is now release-gated deterministically: `CrewStreamingOutput` and `FlowStreamingOutput` propagate run errors during async iteration, mark the stream completed, and keep the same error visible through `result`.
- The latest-upstream empty streaming chunk audit is now release-gated deterministically: zero-length `LLMStreamChunkEvent` content is preserved as a `StreamChunk` with task and agent context instead of being dropped.
...
- Trace backend batch finalization is release-gated with fake Plus API clients: successful sends clear the local event buffer, ephemeral finalization preserves the captured batch id in the trace URL after manager state changes, concurrent backend finalizers serialize to a single Plus API finalize call, and deferred session batches are skipped by shutdown/SIGINT-style finalization checks.
...
- LLM usage normalization mirrors upstream `_usage_to_dict` behavior for dicts, private-field filtering, unsupported primitives, model-like usage dumps, LiteLLM nested cache/reasoning/cache-creation buckets, flat cached-token fields, Anthropic cache-read fields, and absent-bucket preservation without requiring live provider calls.
...
- OpenAI Responses result value-object behavior is release-gated deterministically: `ResponsesAPIResult` initializes empty tool/reasoning buckets, preserves response ids, and exposes upstream `has_tool_outputs` / `has_reasoning` helpers with camelCase aliases; `parse_tool_outputs` and `auto_chain` default to `false`, preserve explicit `true`, auto-chain starts with no previous response id, and explicit `previous_response_id` wins over auto-chain state.
...
- OpenAI/Azure Responses reasoning-chain state is release-gated for upstream empty-state behavior: `last_reasoning_items` is `null` until reasoning items are captured and returns `null` again after reset, auto-chain reasoning include injection avoids duplicating `reasoning.encrypted_content`, and stored reasoning items are prepended to Responses input only when reasoning auto-chain is enabled.
...
- DALL-E image generation docs are intentionally outside the default core gate: upstream examples import `DallETool` from the separate `crewai_tools` package and require live OpenAI image-generation credentials via `client.images.generate`. If this port later carries a `crewai-tools` package, cover it with an optional injected-client shim rather than adding live image API calls to `@crewai-ts/core`.
- Coding-agent `CodeInterpreterTool` injection is intentionally unsupported in the default TypeScript runtime: `allow_code_execution` preserves the agent flag but crew tool preparation emits the deprecation warning and does not add a local code interpreter; dedicated sandbox services remain optional integration scope.
...
- `crewai-files` typed file inputs are represented by deterministic local wrappers for path, bytes, stream, URL, image, PDF, text, audio, and video sources; URL references, provider-specific Bedrock URL fetch fallback, inline base64/bytes resolution, injected-fetch URL reads, direct Agent kickoff and kickoff_async `input_files` attachment to multimodal LLM user messages, Crew kickoff `input_files` attachment to multimodal LLM user messages, Crew kickoff prefer-upload conversion into provider upload blocks, OpenAI/Azure image data URL round trips, OpenAI multiple-image and unsupported-file filtering, Crew-level unsupported OpenAI video fallback through `read_file`, Bedrock direct Agent and provider image/document Converse blocks, Gemini direct Agent text/video and provider image/text/PDF/audio/video `inlineData` blocks with provider text blocks, Anthropic image/document base64 source blocks, and OpenAI multimodal skipping for unsupported text files are release-gated, while live provider uploaders and unmocked remote fetching remain outside the default gate.
- `ReadFileTool` typed-file behavior is release-gated for upstream metadata, required `file_name` args schema, no-file and missing-file messages, multiple text/JSON file lookup, ignored extra kwargs, binary base64 output, and package-neutral PDF text extraction hooks. Core falls back to binary/base64 output when no optional extractor is registered or extraction fails; `@crewai-ts/rag` owns the `pdf-parse` dependency and registers its extractor when imported.
...
- Flow multimodal `input_files` handoff is release-gated for upstream multi-step workflows: files passed to `Flow.kickoff` / `kickoff_async` remain available to crews kicked off from both start methods and downstream listeners, and are cleared after kickoff completes.
...
- MCP AMP reference resolution is release-gated for deterministic fake-config paths: non-HTTPS string refs are bulk-resolved through `_resolve_amp`, unique slugs are fetched once, and `slug#get-page` filters match sanitized tool suffixes such as `get_page`; live CrewAI+ config fetching remains outside the default gate.
...
- ConsoleProvider input handling is release-gated for upstream interactive prompt behavior: both `request_input` and `requestInput` trim whitespace from injected console responses while preserving the provider protocol surface.
- Flow ask provider-error handling is release-gated for upstream async method behavior: provider exceptions inside an async start method resolve to `null` while recording a null response and null response metadata.
- Flow concepts documentation examples are release-gated as deterministic smoke coverage: final-output chaining, state mutation across listeners, `or_` listeners, and `and_` listeners run with local Flow fixtures matching the upstream docs semantics.
...
- StepExecutor string-step execution is release-gated for upstream todo execution parity: public `execute("...")` / `execute_step` calls now propagate caller-provided max-iteration and timeout limits into the isolated text/native action loop instead of falling back to defaults.
...
- CrewAgentExecutor step callback invocation is release-gated for upstream async executor callback semantics: sync step callbacks receive the parsed answer, async callbacks are scheduled from sync invocation without blocking, and missing callbacks are no-ops.
...
- AgentExecutor native tool max-usage handling is release-gated for upstream execution ordering: usage-limited tools record the limit result before invoking deterministic available-function shims, preserve current usage counts, and direct native calls still return a max-usage result when no original tool object is available.
...
2. **Latest upstream baseline advancement**
   ...
   - Snowflake Cortex completion is covered as a deterministic provider shim: token/account URL normalization including preserving existing Cortex API roots, clear missing-token/account errors, and empty explicit keys falling back to env tokens, generic `create_llm("snowflake/...")` native routing, Chat Completions-only API validation, Claude-family message guards, capability flags, and request-parameter shaping are release-gated without live Snowflake credentials.
   ...
   - Flow runtime OR-listener rearming is covered for latest upstream: `_rearm_or_listeners_for_trigger` clears only fired OR listeners whose condition includes the router-emitted trigger and mutates the optional `rearmable` set like upstream, parallel start methods feeding the same OR listener fire it only once, and chained routers within one dispatch wave do not re-fire the same OR listener.
   - Experimental conversational Flow turn helpers are release-gated for deterministic local turns: `handle_turn` / `handleTurn` route through kickoff, `user_message` / `session_id` merge into inputs and state, classifier-free turns clear stale `last_intent`, base `classify_intent` / `classifyIntent` routes through `_collapse_to_outcome` with current user text and conversation context, runtime method tracking resets for each conversational turn, direct kickoff re-runs the graph, `handle_turn` appends returned assistant strings when the handler did not already append one, and `chat` / `chat()` runs an injectable REPL loop over `handle_turn`, stringifies raw outputs, rejects non-conversational flows, and finalizes session traces while restoring deferral state.
   ...
   - Experimental conversational router decisions are release-gated for deterministic local LLM routing: `build_router_context` carries the previous `last_intent`, `route_turn` invokes router LLMs with route response-format metadata, validates returned intents against effective routes, falls back to the configured `fallback_intent` / `fallbackIntent` when the LLM returns an invalid route, persists the selected intent, and `converse_turn` appends chat LLM responses through canonical history.
   ...
   - Experimental conversational flow trace ownership is release-gated for deterministic local action events: `TraceCollectionListener` detects active flow context, treats nested flow execution and flow-owned batches as parent-owned, leaves no-context flow batches out of crew finalization, and claims LLM action trace batches with flow owner id/name metadata without finalizing nested crew work.
...
- `Converter.ato_json` / `atoJson` now mirrors upstream native-async output conversion safety by keeping non-function-calling paths on async `acall` and deferring sync-only function-calling instructor work out of the immediate microtask path before returning JSON.
...
- System signal events are release-gated for upstream adapter/decorator semantics: `SIGNAL_EVENT_TYPES` lists all supported signal event classes, `signal_event_adapter.validate_python` / `validatePython` restores signal payloads by discriminated `type`, invalid signal types are rejected, signal events expose `model_dump` / `modelDump` round-trip serialization, and `on_signal` registers handlers for all supported signal events while returning the original handler.
...
- Task output file handling now mirrors upstream safety and filesystem behavior: `output_file` rejects path traversal, shell expansion including `$HOME` and `~`, shell metacharacters, and invalid template variables; `create_directory=false` raises a clear directory-missing error instead of surfacing raw filesystem `ENOENT`, while still writing when the target directory already exists, and crew kickoff writes interpolated task output files end-to-end.
...
- Latest-upstream `flow/flow_definition` schema models are release-gated as deterministic local contract utilities: Flow definition/config/state/method/persistence/HITL/diagnostic classes, `to_dict` / `to_json` / `to_yaml` serialization, `from_dict` / `from_json` / `from_yaml` round trips, JSON schema metadata, upstream validation diagnostics for router triggers, explicit router emits, dynamic routers without static emits, static DSL method-definition fragments including decorator-attached fragments and namespace fragments, `@start("event")` / callable / string / nested condition sugar projection, legacy DSL metadata fallback, static class config/description fields, `extract_flow_definition` registry projection, human-feedback stacked fragment preservation, static human-feedback method attributes, human-feedback outcome/LLM consistency, human-feedback emit overriding inner router emit, non-serializable human-feedback metadata refs, class/method `@persist` metadata, conversational builtin method contracts, serialized diagnostic preservation through `with_diagnostics`, `Flow.flow_definition()` lazy cache behavior that does not leak parent definitions into subclasses, loaded FlowDefinition visualization overrides such as explicit `start: false`, direct FlowDefinition visualization structure building without runtime source/signature metadata, static visualization router event aliases, router events to direct OR string listener conditions while excluding AND-nested conditions, visualization routers that stay dynamic when no explicit/static emit exists, static-contract orphaned trigger warnings, plus warning suppression when explicit router events cover all static listener triggers.
...
- `BaseTool.run` / `arun` now have default-gate coverage for async execution: promise-like `_run` outputs are settled before cache writes and usage-finished events, later cache hits return the stored value without incrementing usage or reinvoking the tool, and concurrent `arun` calls can all enter async `_arun` before completion while preserving per-call results and usage counts.
...
- `ToolUsage._check_usage_limit` now has default-gate coverage for upstream limit-check progression: checks return null while `current_usage_count` is below `max_usage_count`, return the upstream limit message at the threshold, and do not mutate the counter themselves.
...
- `CrewTrainingHandler.append` now has default-gate coverage for upstream per-agent iteration bucket behavior: appending new training data for a second agent creates an isolated agent bucket without disturbing existing saved data or prior agent iterations.
...
- Legacy Agent `reasoning=true` compatibility now creates an upstream-style default `PlanningConfig`, keeps planning enabled, preserves `max_attempts=null` when no legacy max is provided, and still honors explicit `max_reasoning_attempts`.
- Agent `execute_task` without planning now has default-gate coverage for upstream no-planning semantics: task descriptions are passed through without injecting `Planning:` text and the original task object remains unchanged.
- Agent `execute_task` with `planning_config` now release-gates upstream planning injection semantics: a generated plan is inserted into the execution prompt while the original task description remains unchanged.
...
- Crew planning summaries now read upstream direct `agent.knowledge_sources` as well as initialized local knowledge stores, preserve deterministic agent knowledge context in planner prompts, and format upstream agent tool lists plus the `"agent has no tools"` fallback for planner task summaries.
...
- `AgentExecutor.observe_step_result` now respects upstream `PlanningConfig.observe_steps` and `reasoning_effort`: medium/high run planner observation by default, low and explicit `observe_steps=false` use heuristic observation, failed low-effort observations mark the running todo failed without triggering replans, and observation audit logs record whether an LLM observation occurred.
...
- Flow HITL learning now has deterministic local behavior for injected LLM clients: recalled memory lessons can pre-review method output before provider display, non-empty feedback can distill new lessons into flow memory, and non-strict pre-review/distillation failures emit upstream-style warnings while preserving the flow output.
...
- `toSerializable` now mirrors upstream BaseModel serialization by honoring `modelDump` / `model_dump` output before object entry serialization, with recursive exclude handling and upstream-style Python repr output when max-depth truncation is reached.
  ...
  - The latest-upstream `inputs.id` deprecation migration guide is release-gated deterministically: deprecated `inputs.id` hydration resumes and writes under the supplied flow id, while `restore_from_state_id` hydrates from the source snapshot, preserves that source record, and writes under a fresh `state.id`.
...
- Added adapter-level LLM provider parity helpers:
  ...
  - OpenAI native completion shim now exposes upstream-style SDK response token usage extraction with absent cached/reasoning detail omission, Responses API result value-object helpers, parse-tool-output and auto-chain flags, explicit previous-response precedence, reasoning include de-duplication and input prepending, output parsing for function calls, built-in tool outputs, and reasoning items, and deterministic Responses streaming event accumulation, including omission of absent cached/reasoning token detail fields plus present cached prompt tokens and reasoning tokens
  ...
  - Gemini native provider detection now routes `google/...` and `gemini/...` model aliases through `GeminiCompletion` with upstream-style Gemini 2.x family detection.
...
- Added A2A update-handler compatibility behavior:
  ...
  - A2A server task execution now emits upstream-style started/completed lifecycle events around deterministic task execution while preserving completed task enqueue semantics.
  ...
  - A2A `http_url_adapter` now exposes upstream-style `validate_python` / `validatePython` helpers for strict HTTP/HTTPS URL payload validation and WHATWG URL normalization.
...
3. Flow and persistence
   ...
   - Cyclic Flow router loops now have default-gate coverage for upstream OR listener reset behavior, ensuring merge listeners and loop-back listeners fire once per iteration, persisted flows with explicit input ids continue all loop iterations, direct self-listening methods fail with an infinite-loop error, `or_()` self-listening methods do not re-trigger themselves indefinitely, and conditional routers activate only the returned event instead of all possible listener events.
...
6. Evaluation and tracing listeners
   ...
   - Flow-contained crew telemetry is release-gated for upstream local span lifecycle: multiple `share_crew=true` crews kicked off from start/listener flow methods each receive distinct `_execution_span` objects and close them after deterministic local LLM execution.
