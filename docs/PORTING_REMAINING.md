...
- Added MCP compatibility behavior:
  ...
  - `create_model_from_schema` now returns an upstream-style model-like schema validator for MCP/tool JSON schemas, including required fields, optional `null` defaults, enum/const rejection, string/numeric field constraints, nested objects, arrays, `$ref`, `allOf`, `anyOf`/`oneOf`, and enriched field descriptions.
