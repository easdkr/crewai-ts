...
3. Flow and persistence
   ...
   - Cyclic Flow router loops now have default-gate coverage for upstream OR listener reset behavior, ensuring merge listeners and loop-back listeners fire once per iteration while `or_()` self-listening methods do not re-trigger themselves indefinitely.
