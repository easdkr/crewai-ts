...
| Experimental skills repository | Deterministically covered, network download shimmed | Local discovery, cache, archive extraction, metadata, and experimental feature gating are release-gated. Registry downloads should use fake API/archive fixtures only. | Live CrewAI+ registry downloads and authenticated platform calls are optional integration scope only. |
...
- Flow event causal ordering is release-gated for listener chains, OR-condition listeners, router paths, and parallel listeners: downstream method execution events preserve upstream `triggered_by_event_id` links to the exact method completion event that caused them.
