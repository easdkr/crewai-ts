...
- OpenAI Responses result value-object behavior is release-gated deterministically: `ResponsesAPIResult` initializes empty tool/reasoning buckets, preserves response ids, and exposes upstream `has_tool_outputs` / `has_reasoning` helpers with camelCase aliases.
...
- Added adapter-level LLM provider parity helpers:
  ...
  - OpenAI native completion shim now exposes upstream-style SDK response token usage extraction, Responses API result value-object helpers, output parsing for function calls, built-in tool outputs, and reasoning items, and deterministic Responses streaming event accumulation, including cached prompt tokens and reasoning tokens
