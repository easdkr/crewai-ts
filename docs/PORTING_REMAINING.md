...
- OpenAI/Azure Responses reasoning-chain state is release-gated for upstream empty-state behavior: `last_reasoning_items` is `null` until reasoning items are captured and returns `null` again after reset, and auto-chain reasoning include injection avoids duplicating `reasoning.encrypted_content`.
...
- Added adapter-level LLM provider parity helpers:
  ...
  - OpenAI native completion shim now exposes upstream-style SDK response token usage extraction, Responses API result value-object helpers, parse-tool-output and auto-chain flags, explicit previous-response precedence, reasoning include de-duplication, output parsing for function calls, built-in tool outputs, and reasoning items, and deterministic Responses streaming event accumulation, including cached prompt tokens and reasoning tokens
