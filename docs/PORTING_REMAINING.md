...
- Added unified memory compatibility helpers used by Flow and scoped views:
  ...
  - `EncodingFlow.batch_embed` and `intra_batch_dedup` now expose upstream-style batch embedding and vector-similarity duplicate marking for exported memory flow compatibility.
