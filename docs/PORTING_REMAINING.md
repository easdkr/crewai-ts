...
- Crew replay is release-gated for upstream stored-context recovery: replaying a later task restores earlier task outputs and messages from the task-output storage handler before building task context.
...
- Hierarchical manager delegation tools are release-gated for upstream coworker scoping, manager validation, and tool descriptions: manager agents cannot be included in the regular agents list or carry their own tools, assigned-task delegation exposes only that task's agent, unassigned tasks expose all crew agents, and manager tool descriptions use the upstream "specific task/question" coworker wording.
...
- Crew for-each streaming output is release-gated for upstream kickoff result semantics: stream-mode `kickoffForEach`, `kickoffForEachAsync`, and `akickoffForEach` preserve all per-input `CrewOutput` results while exposing the last output as the stream chunk source and aggregating usage on the parent crew.
