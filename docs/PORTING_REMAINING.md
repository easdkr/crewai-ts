...
- The same latest-upstream FlowDefinition contract audit is now release-gated deterministically: Flow DSL condition dictionaries validate only `type` / `conditions` / `methods`, reject string-valued condition/method sequences and unknown keys, and normalize upstream `methods` fragments into runtime condition lists.
