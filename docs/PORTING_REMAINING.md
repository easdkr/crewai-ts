...
- AgentExecutor parallel planning execution is release-gated for upstream StepExecutor semantics: ready todos are marked running, executed through isolated StepExecutor contexts, recorded in step execution logs, and left for observation/finalization instead of being completed by the parallel executor stub.
...
- `AgentExecutor.execute_todos_parallel` now executes ready planning todos through isolated `StepExecutor` contexts and records upstream-style step execution audit fields while leaving todo completion decisions to observation/finalization.
