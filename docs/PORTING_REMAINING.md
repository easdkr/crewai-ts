...
- ToolUsage input parsing is release-gated for upstream malformed LLM argument recovery: Python literal booleans/nulls and single quotes, JSON5-style unquoted object keys, trailing commas in objects/arrays, nested structures, and invalid-input event emission are covered without adding a JSON5 runtime dependency.
