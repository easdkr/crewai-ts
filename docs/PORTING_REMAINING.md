...
- Standard task decorators are release-gated for upstream async factory memoization and direct-call name inference: directly awaited async task factories return the same cached task instance and assign the method name when no explicit task name is present.
